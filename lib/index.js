...
async function transcribeWithChatAsr(audioBuffer, asr) {
	...
	if (!result.ok) {
		...
		const err = new Error(`ASR 接口错误 ${result.status}: ${detail}`);
		err.status = 502;
		throw err;
	}
	if (result.text.trim() === "" || Object.keys(body).length === 0) {
		const err = new Error(`ASR 接口返回空响应（${describeResponse(result)}）`);
		err.status = 502;
		throw err;
	}
...
async function transcribe(audioBuffer, asr) {
	...
	const resp = await fetch(`${asr.baseUrl.replace(/\/+$/, "")}/audio/transcriptions`, {
		method: "POST",
		headers: { Authorization: `Bearer ${asr.apiKey}`, ...NO_COMPRESSION_HEADERS },
		body: form
	});
	...
	if (raw.trim() === "") {
		const err = new Error(`ASR 接口返回空响应（HTTP ${resp.status} content-type=${resp.headers.get("content-type") || "-"}）`);
		err.status = 502;
		throw err;
	}
 ...
 * @param text - 待合成文本。
 * @param tts - TTS 配置 { baseUrl, model, apiKey, voice }。
...
async function synthesizeWithCustomTts(text, tts) {
	...
	const voice = String(tts.voice ?? "").trim() || "alloy"; // 每个引擎自己的音色槽
	...
	if (!resp.ok) {
		const raw = await resp.text().catch(() => "");
		let detail = raw.slice(0, 200) || "（空响应）";
		try {
	...
	if (audio.length === 0) {
		const err = new Error(`自定义 TTS 返回空音频（HTTP ${resp.status} content-type=${resp.headers.get("content-type") || "-"} content-encoding=${resp.headers.get("content-encoding") || "-"}）`);
		err.status = 502;
		throw err;
	}
	return audio;
...
async function synthesizeWithMimoTts(text, tts) {
	...
	const body = result.json;
	if (!result.ok) {
		const detail = Object.keys(body).length > 0
			? (body.error?.message ?? JSON.stringify(body))
			: (result.text.slice(0, 200) || "（空响应）");
		const err = new Error(`MiMo TTS 接口错误 ${result.status}: ${detail}`);
		err.status = 502;
		throw err;
	}
	const message = Array.isArray(body.choices) ? body.choices[0]?.message : null;
	...
	if (typeof data !== "string" || data === "") {
		const err = new Error(`MiMo TTS 响应里没有音频数据（${describeResponse(result)}）`);
		err.status = 502;
		throw err;
	}
