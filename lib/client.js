window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const ASR_ENGINES = ["siliconflow", "groq", "mimo", "custom"];
		...
		function VoiceChatSettingsSection(props) {
			...
			const save = () => {
				...
				const asrPatch = { baseUrl: trim(asrSlot.baseUrl), model: trim(asrSlot.model), apiKey: trim(asrSlot.apiKey) };
			...
			return React.createElement("div", { style: SECTION_STYLES.wrap },
				...
				field("ASR 引擎",
					...
					engine === "mimo"
						...
						: "OpenAI 兼容 /audio/transcriptions multipart 协议，录音 webm 原样上传；每个引擎的配置各自独立"),
				field("ASR Base URL",
				...
				field("ASR API Key",
					...
					`只作用于「${engine}」这一个 ASR 引擎；保存在 settings.local.json，留空则沿用服务端同引擎已配密钥`),
				...
				ttsEngine === "custom" ? React.createElement(React.Fragment, null,
					...
					field("朗读音色（自定义 TTS）",
						...
						"接口的 voice 参数（如 alloy / zh-CN-XiaoxiaoNeural / Kokoro 音色名）；留空用 alloy")
				) : null,
				...
				ttsEngine === "mimo" ? React.createElement(React.Fragment, null,
					field("TTS Base URL",
						...
						`MiMo 基地址（自动拼 /chat/completions）或完整端点均可；留空用内置默认 ${TTS_ENGINE_DEFAULTS.mimo.baseUrl}`),
					field("TTS 模型名称",
						React.createElement("input", {
							style: SECTION_STYLES.input, value: ttsSlot.model || "", spellCheck: false,
					...
					field("TTS API Key",
						...
						"只作用于「MiMo TTS」；留空则沿用 MiMo ASR 那一份的密钥（同厂商）"),
					field("朗读音色（MiMo TTS）",
						React.createElement("input", {
				...
				ttsEngine === "edge" ? field("朗读音色（Edge TTS）",
					...
					"只作用于「Edge TTS」；留空用内置默认晓晓，下拉是常用音色，也可输入任意有效音色名"
				) : null,
		...
		function VoiceChatButton(props) {
			...
			const playOne = useCallback(async (text) => {
				...
				if (!spoken) setHint(brief ? `朗读失败：${brief}` : "朗读失败（服务端与浏览器均无声音）");
				else if (brief) setHint(`已降级为浏览器朗读：${brief}`);
				return spoken;
			...
			return React.createElement("span", { style: styles.wrap },
				...
				React.createElement("button", {
					...
					style: styles.mic,
					disabled: busy,
					title: recording ? "点击结束并发送" : busy ? "转写中…" : "点击开始聆听（静音自动结束）· 快捷键 Ctrl+Shift+Space"
				}, React.createElement(Icon, { paths: ICON_MIC, size: 16 })),
				React.createElement("button", {
					type: "button",
					onClick: toggleMute,
